...
const empty = { alt_ar: "", alt_en: "", image: "" };

export default function AdminFleetPage({ params: { locale } }: { params: { locale: string } }) {
  const isRtl = locale === "ar";
  const [fleet, setFleet] = useState<FleetItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [modal, setModal] = useState<{ open: boolean; editing: FleetItem | null }>({ open: false, editing: null });
  ...
  const openEdit = (item: FleetItem) => {
    setForm({ alt_ar: item.alt_ar, alt_en: item.alt_en, image: item.image });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    if (modal.editing) {
      await fetch("/api/fleet", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: modal.editing.id, ...form }) });
    } else {
      await fetch("/api/fleet", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    }
    setSaving(false);
    closeModal();
    fetchFleet();
  ...
  const handleToggleVisible = async (item: FleetItem) => {
    const updated = !item.visible;
    setFleet((prev) => prev.map((x) => (x.id === item.id ? { ...x, visible: updated } : x)));
    await fetch("/api/fleet", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id, visible: updated }) });
  };
  ...
  const visibleCount = fleet.filter((f) => f.visible).length;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-primary">{isRtl ? "إدارة الأسطول" : "Fleet Management"}</h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {isRtl ? `${visibleCount} صورة مفعّلة` : `${visibleCount} images visible`}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          <p className="text-slate-400 font-bold">{isRtl ? "لا توجد صور بعد" : "No fleet images yet"}</p>
          <button onClick={openAdd} className="mt-4 text-primary font-bold text-sm hover:underline">{isRtl ? "أضف أول صورة" : "Add your first image"}</button>
        </div>
      ) : (
            ...
            <div key={item.id} className={`bg-white rounded-xl border border-slate-200 overflow-hidden ${!item.visible ? "opacity-50" : ""}`}>
              ...
              <div className="p-4 space-y-3">
                <div className="min-w-0">
                  <p className="font-bold text-sm text-primary truncate">{item.alt_ar}</p>
                  <p className="text-xs text-slate-400 truncate mt-0.5">{item.alt_en}</p>
                </div>
          ...
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100">
              <h3 className="text-base sm:text-lg font-black text-primary">
                {modal.editing ? (isRtl ? "تعديل الصورة" : "Edit Image") : (isRtl ? "إضافة صورة جديدة" : "Add New Image")}
              </h3>
            ...
            <form onSubmit={handleSave} className="p-4 sm:p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5">{isRtl ? "الوصف (عربي)" : "Alt Text (Arabic)"}</label>
                  <input required dir="rtl" value={form.alt_ar} onChange={(e) => setForm({ ...form, alt_ar: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-primary/20 font-medium text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5">{isRtl ? "الوصف (إنجليزي)" : "Alt Text (English)"}</label>
                  <input required dir="ltr" value={form.alt_en} onChange={(e) => setForm({ ...form, alt_en: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-primary/20 font-medium text-sm" />
                </div>
              </div>
              ...
              <button type="submit" disabled={saving || !form.image} className="w-full bg-primary text-white py-3.5 rounded-xl font-black text-sm hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                {modal.editing ? (isRtl ? "حفظ التعديلات" : "Save Changes") : (isRtl ? "إضافة الصورة" : "Add Image")}
              </button>
            </form>
          </div>
        </div>
